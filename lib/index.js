...
class Klaviyo {
    ...
    return new Promise((resolve, reject) => {
      axios({
        ...
        params: {
          ...
          .catch((error) => {
            this.logger.log('error', `Encountered ${error.response.status} \
              from GET to ${endpoint}`, data);
            reject(error);
          });
    });
  }
   ...
   * for this user
   * @param {Oject} [eventProperties={}] - The string "foo"
   * @param {Oject} [customerProperties={}] - The string "foo"
   */
   ...
   * @param {String} email - The email that acts as a unique identified
   * for this user
   * @param {Oject} [customerProperties={}] - Custom information about
